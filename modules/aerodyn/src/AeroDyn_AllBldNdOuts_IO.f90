      ...
      DO IdxChan=1,p%BldNd_NumOuts
         ...
         CASE ( BldNd_Theta )
            ...
            elseif (p_AD%WakeMod /= WakeMod_DMST) then
               ...
               DO IdxBlade=1,p%BldNd_BladesOut
                  DO IdxNode=1,p%NumBlNds                   
                     y%WriteOutput( OutIdx )  = m_AD%rotors(iRot)%DMST_u%PitchAndTwist(IdxNode,IdxBlade)*R2D
                     OutIdx = OutIdx + 1
                  END DO
               END DO
            endif
...
!NOT available in FVW or DMST yet
         ...
         CASE ( BldNd_Cn )
            ...
            elseif (p_AD%WakeMod /= WakeMod_DMST) then
               ...
               DO IdxBlade=1,p%BldNd_BladesOut
                  DO IdxNode=1,p%NumBlNds                   
                     cp=cos(m_AD%rotors(iRot)%blds(IdxBlade)%BN_phi(IdxNode))    ! cos(phi)
                     sp=sin(m_AD%rotors(iRot)%blds(IdxBlade)%BN_phi(IdxNode))    ! sin(phi)
                     y%WriteOutput( OutIdx )  = m_AD%rotors(iRot)%blds(IdxBlade)%BN_Cl(IdxNode)*cp + m_AD%rotors(iRot)%blds(IdxBlade)%BN_Cd(IdxNode)*sp
                     OutIdx = OutIdx + 1
                  END DO
               END DO
            endif
            ...
            elseif (p_AD%WakeMod /= WakeMod_DMST) then
               ...
               DO IdxBlade=1,p%BldNd_BladesOut
                  DO IdxNode=1,p%NumBlNds                   
                     cp=cos(m_AD%rotors(iRot)%blds(IdxBlade)%BN_phi(IdxNode))    ! cos(phi)
                     sp=sin(m_AD%rotors(iRot)%blds(IdxBlade)%BN_phi(IdxNode))    ! sin(phi)
                     y%WriteOutput( OutIdx )  = m_AD%rotors(iRot)%blds(IdxBlade)%BN_Cd(IdxNode)*cp - m_AD%rotors(iRot)%blds(IdxBlade)%BN_Cl(IdxNode)*sp
                     OutIdx = OutIdx + 1
                  END DO
               END DO
            endif
...
!NOT available in FVW or DMST yet
         ...
         CASE ( BldNd_CpMin )
            if (p_AD%WakeMod /= WakeMod_FVW .and. p_AD%WakeMod /= WakeMod_DMST) then
               ...
               DO IdxBlade=1,p%BldNd_BladesOut
                  DO IdxNode=1,u%BladeMotion(IdxBlade)%NNodes
                     y%WriteOutput( OutIdx ) = m_AD%rotors(iRot)%blds(IdxBlade)%BN_Cpmin(IdxNode)
                     OutIdx = OutIdx + 1
                  ENDDO
               ENDDO
            endif
         ...
         CASE ( BldNd_Gam )
            ...
            elseif (p_AD%WakeMod /= WakeMod_DMST) then
               ...
               DO IdxBlade=1,p%BldNd_BladesOut
                  DO IdxNode=1,u%BladeMotion(IdxBlade)%NNodes
                     y%WriteOutput( OutIdx ) = 0.5_ReKi * p_AD%rotors(iRot)%DMST%chord(IdxNode,IdxBlade) * m_AD%rotors(iRot)%blds(IdxBlade)%BN_Vrel(IdxNode) * m_AD%rotors(iRot)%blds(IdxBlade)%BN_Cl(IdxNode) ! "Gam" [m^2/s]
                     OutIdx = OutIdx + 1
                  ENDDO
               ENDDO
            endif
...
!NOT available in BEMT/DBEMT yet
         ...
         CASE ( BldNd_Uin )
            ...
            elseif (p_AD%WakeMod /= WakeMod_DMST) then
               ...
               DO IdxBlade=1,p%BldNd_BladesOut
                  DO IdxNode=1,u%BladeMotion(IdxBlade)%NNodes
                     y%WriteOutput( OutIdx ) = dot_product(M_pg(1,1:3,IdxBlade), m_AD%rotors(iRot)%DMST_y%Vind(1:3,IdxNode,IdxBlade) ) ! Uihn, hub normal
                     OutIdx = OutIdx + 1
                  ENDDO
               ENDDO
            endif
         ...
         CASE ( BldNd_Uit )
            ...
            elseif (p_AD%WakeMod /= WakeMod_DMST) then
               ...
               DO IdxBlade=1,p%BldNd_BladesOut
                  DO IdxNode=1,u%BladeMotion(IdxBlade)%NNodes
                     y%WriteOutput( OutIdx ) = dot_product(M_pg(2,1:3,IdxBlade), m_AD%rotors(iRot)%DMST_y%Vind(1:3,IdxNode,IdxBlade) ) ! Uiht, hub tangential
                     OutIdx = OutIdx + 1
                  ENDDO
               ENDDO
            endif
         ...
         CASE ( BldNd_Uir )
            ...
            elseif (p_AD%WakeMod /= WakeMod_DMST) then
               ...
               DO IdxBlade=1,p%BldNd_BladesOut
                  DO IdxNode=1,u%BladeMotion(IdxBlade)%NNodes
                     y%WriteOutput( OutIdx ) = dot_product(M_pg(3,1:3,IdxBlade), m_AD%rotors(iRot)%DMST_y%Vind(1:3,IdxNode,IdxBlade) ) ! Uihr, hub radial
                     OutIdx = OutIdx + 1
                  ENDDO
               ENDDO
            endif
